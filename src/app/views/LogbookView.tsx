...
export const LogbookView: React.FC = () => {
  ...
  const fetchData = async (showSkeleton = true) => {
    ...
    try {
      // Fetch logbooks
      const logRes = await fetch(`${API_BASE}/logbooks/mine`, { headers: getAuthHeaders() });
      const logData = await logRes.json();
      if (logData.success) {
        setLogs(logData.logbooks);
      }

      // Fetch student dashboard to get joined clubs
  ...
  useEffect(() => {
    ...
    // Re-fetch when tab becomes visible (picks up club head approvals/rejections)
      ...
      {loading ? (
        ...
        >
          ...
          <p className="text-sm text-gray-400 mt-1">
            {statusFilter !== 'All' ? `No ${statusFilter.toLowerCase()} entries.` : 'Start by submitting your first activity log!'}
          </p>
        </div>
      ) : (
        <div
            ...
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-gray-50 text-sm">
                {filteredLogs.map(log => {
                  const clubName = getClubName(log);
                  return (
